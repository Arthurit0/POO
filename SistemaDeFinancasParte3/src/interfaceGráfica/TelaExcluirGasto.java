    ...
    private void jButtonConfirmRmvActionPerformed(java.awt.event.ActionEvent evt) {                                                  
        ...
        if(selected != "IDs"){
            ...
            try {
                ...
                JOptionPane.showMessageDialog(jPanel, "Gasto excluído com sucesso!", "Sucesso!", JOptionPane.INFORMATION_MESSAGE);
                backToMenu();
            } catch (DeleteException | SelectException e) {
                JOptionPane.showMessageDialog(jPanel, e.getMessage(), "Erro!", JOptionPane.ERROR_MESSAGE);
            }
        }else{
            JOptionPane.showMessageDialog(jPanel, "Nenhum ID selecionado", "Sem seleção!", JOptionPane.OK_CANCEL_OPTION);
        }
    }
